...
//       //var destination = '/admin/home/55a44344480b4c2a08f7aea6';
    ...
    function ($rootScope, $http, $location, $stateParams, $cookies, $q, $timeout, $cookieStore) {
        ...
        MeanUserKlass.prototype.onIdentity = function (response) {
            ...
            if (!response) {
                ...
                var destination = $cookies.redirect;
                if (this.user.roles.indexOf('admin') !== -1) this.isAdmin = true;
                if (this.user.roles.indexOf('school_admin') !== -1) this.isSchoolAdmin = true;
                $rootScope.$emit('loggedin');
                if (destination) {
                    $location.path(destination.replace(/^"|"$/g, ''));
                    $cookieStore.remove('redirect');
        ...
        MeanUserKlass.prototype.checkSchoolAdmin = function () {
            var deferred = $q.defer();

            // Make an AJAX call to check if the user is logged in
            $http.get('/api/loggedin').success(function (user) {
                // Authenticated
                if (user !== '0' && user.roles.indexOf('school_admin') !== -1) {
        ...
        MeanUserKlass.prototype.checkRoles = function (roles) {
            var deferred = $q.defer();
